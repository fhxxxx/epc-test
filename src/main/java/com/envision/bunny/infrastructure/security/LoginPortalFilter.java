...
import com.envision.bunny.infrastructure.security.aad.AadProperties;
import com.envision.bunny.infrastructure.security.okta.OktaProperties;
import com.envision.bunny.infrastructure.security.wecom.WecomProperties;
...
@Order(2)
@Component
@EnableConfigurationProperties({OktaProperties.class,WecomProperties.class, AadProperties.class})
@Slf4j
...
public class LoginPortalFilter extends OncePerRequestFilter {
    ...
    private String buildAadRedirectUrl(String stateUrl) {

        return UrlBuilder.of(aadProperties.getAuthorizationEndpoint())
                .addQuery("client_id", aadProperties.getClientId())
                .addQuery("response_type", RESPONSE_TYPE)
                .addQuery("redirect_uri", aadProperties.getRedirectUri())
                .addQuery("scope", aadProperties.getScope())
                // .addQuery("scope", "openid offline_access https://graph.microsoft.com/user.read")
                .addQuery("state", stateUrl)
                .addQuery("nonce", NONCE)
                .setCharset(StandardCharsets.UTF_8)
                .build();
